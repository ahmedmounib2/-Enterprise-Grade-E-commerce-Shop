...
- `settlements_payouts_retryable_count` (counter): number of retry-eligible payouts discovered by
  the payout retry cron.
- `settlements_payouts_retryable_attempt` (counter): number of retry attempts executed.
- `cron_settlement_scheduler_run` (counter with `status` label): scheduler lifecycle outcomes
  (`started`, `success`, `failed`).
- `cron_settlement_scheduler_last_run_epoch_ms` (counter-style snapshot): last scheduler run
...
Use job health (or `cron_settlement_scheduler_run{status="failed"}` equivalents) to trigger when
...
These mirror existing backend stale/failure checks in `jobHealth.service.js` and provide external
observability hooks for Prometheus/Datadog dashboards.
