...
This workspace contains the customer-facing React single-page application that powers AhmedMonib
E-Shop. It is built with Vite for rapid iteration, Tailwind CSS + DaisyUI for styling, and a thin
theming bridge in [`frontend/theme/`](./theme/) that syncs with the shared DaisyUI catalogue defined
at the monorepo level. Runtime data flows through the shared
[`@eshop/api-client`](../packages/api-client) package, which wraps the Express backend API
(`backend/`) and ships typed helpers, Axios configuration, and authenticated request utilities. This
